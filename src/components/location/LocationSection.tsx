      ...
      <div className="flex items-start gap-1 ">
        ...
        <div>
          <div className="font-poppinsRegular text-2xl ">
            Shipped from <span className="font-poppinsBold font-bold">{location.city}</span>
          </div>
          {location.description.map((line, index) => (
            <p key={index} className="text-lg my-1">{line}</p>
          ))}
        </div>
      </div>
      ...
      <div className="mt-8 pl-16 md:pl-32">
        ...
        <div className="font-poppinsRegular flex items-center gap-2 translate-y-6 translate-x-6 mb-5 ml-32">
          <img 
            ...
            alt="WhatsApp" 
            className="w-[43px] h-auto translate-x-3 ml"
          />
          <span className="text-lg">{location.whatsapp.name} {location.whatsapp.number}</span>
        </div>
        
        {/* Address */}
        <div className="flex items-start gap-3">
          <img 
            src="/src/assets/Icons/GMAPS.png" 
            alt="Google Maps" 
            className="w-40 h-auto"
          />
          <div>
            <span className="font-poppinsBold font-Bold">{location.address.name}</span>
            {location.address.lines.map((line, index) => (
              <p key={index} className="font-poppinsRegular text-lg">{line}</p>
            ))}
          </div>
        </div>
      </div>
