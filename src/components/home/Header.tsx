...
const Header = ({ cartCount, cartItems, onSearch }: HeaderProps) => {    
  ...
  // State untuk WhatsApp button position - IMPROVED VERSION
  const [whatsappPosition, setWhatsappPosition] = useState({
    x: typeof window !== 'undefined' ? window.innerWidth - 80 : 300,
    y: typeof window !== 'undefined' ? window.innerHeight / 2 : 300,
  });

  const [isDragging, setIsDragging] = useState(false); // Gunakan state untuk re-render
  ...
  // WhatsApp button drag functionality - IMPROVED VERSION
  ...
  const handleDragMove = (e: MouseEvent | TouchEvent) => {
    ...
    if (isDragging || (Math.abs(deltaX) > 8 || Math.abs(deltaY) > 8)) {
      ...
      // Boundary checks dengan buffer yang lebih baik
  ...
  const handleDragEnd = (e: Event) => {
    ...
    if (isDragging) {
      ...
      // Snap to nearest edge dengan threshold yang lebih baik
      const snapThreshold = window.innerWidth / 2;
      const currentX = whatsappPosition.x;
      const shouldSnapLeft = currentX < snapThreshold;
      
      // Gunakan posisi terakhir yang sudah di-update, bukan dari state lama
    ...
    // Reset dragging state dengan delay kecil untuk mencegah flash
    setTimeout(() => {
      setIsDragging(false);
      isClick.current = true;
    }, 50);
...
{/* WhatsApp Floating Button - IMPROVED VERSION */}
<div
  ref={whatsappRef}
  className={`fixed z-40 rounded-full p-3 shadow-lg transition-all duration-200 ${
    isDragging 
      ? 'scale-110 shadow-xl cursor-grabbing active:scale-110 bg-white border-2 border-[#dcbec1]' // Warna putih dengan border saat drag
      : 'hover:scale-105 cursor-pointer active:scale-95 bg-[#dcbec1] hover:bg-[#d4b6b9]' // Warna normal dan hover
  }`}
  style={{
...
>    
  <svg    
    className="w-6 h-6 ${
      isDragging ? 'text-[#dcbec1]' : 'text-gray-800'
    }"    
    fill="currentColor"    
    viewBox="0 0 24 24"    
